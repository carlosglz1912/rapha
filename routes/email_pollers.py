                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Rapha (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Rapha" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "rapha_kind" if "rapha_kind" in cols else "'scheduled' AS rapha_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                rapha_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Rapha-Origin"] = "rapha-ui"
                outer["X-Rapha-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", rapha_kind or "scheduled")[:64]
                outer["X-Rapha-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `rapha-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `RAPHA_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `rapha-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("RAPHA_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `RAPHA_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (RAPHA_INPROCESS_POLLERS=0); "
            "drive `rapha-mail poll-scheduled` externally."
        )
        return
    import asyncio
