...
# --- Non-rapha services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_rapha_services_match_base(base, standalone_path):
...
# --- rapha = base service + only the overlay additions ------------------


def test_nvidia_rapha_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_rapha_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
