...
def test_vllm_preflight_reports_cli_and_version():
    ...
    assert 'RAPHA_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"' in script
    assert 'echo "[rapha] vLLM CLI: $RAPHA_VLLM_BIN"' in script
    assert '"$RAPHA_VLLM_BIN" --version' in script
    assert 'RAPHA_PREFLIGHT_EXIT=127' in script
...
def test_serve_preflight_failure_keeps_tmux_pane_visible():
    ...
    assert 'RAPHA_PREFLIGHT_EXIT=127' in script
    assert 'echo "=== Process exited with code $RAPHA_PREFLIGHT_EXIT ==="' in script
...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "RAPHA_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $RAPHA_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$RAPHA_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_rapha_has_cudart' in script
    ...
    assert script.index('_rapha_has_cudart') < script.index('DGGML_CUDA=ON')
