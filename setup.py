...
def create_default_admin():
    ...
    try:
        ...
        from core.rapha_env import get_env

        username = get_env("RAPHA_ADMIN_USER", "ODYSSEUS_ADMIN_USER").lower()
        password = get_env("RAPHA_ADMIN_PASSWORD", "ODYSSEUS_ADMIN_PASSWORD")

        if username and password:
            # Both provided via env — use them directly
            pass
        elif sys.stdin.isatty() and not os.getenv("RAPHA_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("RAPHA_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("RAPHA_ADMIN_PASSWORD"):
...
def main():
    ...
    if not os.getenv("RAPHA_SKIP_RUN_HINT"):
