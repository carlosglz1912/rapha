...
# Shell overrides (RAPHA_PORT / RAPHA_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${RAPHA_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${RAPHA_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
# RAPHA_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$RAPHA_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
