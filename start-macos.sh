#!/bin/bash
# Rapha — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Rapha needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Rapha
...
# Shell overrides (RAPHA_PORT / RAPHA_HOST) take top priority, followed by
# legacy ODYSSEUS_* names, .env APP_* values, and built-in defaults.
PORT="${RAPHA_PORT:-${ODYSSEUS_PORT:-${APP_PORT:-7860}}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${RAPHA_HOST:-${ODYSSEUS_HOST:-${APP_BIND:-127.0.0.1}}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Rapha quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    RAPHA_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
echo "▶ Preparing Rapha…"
RAPHA_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/rapha-apfel.log"
...
if (exec 3<>"/dev/tcp/127.0.0.1/$CHROMA_PORT") 2>/dev/null; then
    ...
    CHROMA_LOG="${TMPDIR:-/tmp}/rapha-chromadb.log"
...
# RAPHA_NO_OPEN=1 (or legacy ODYSSEUS_NO_OPEN=1) over SSH / headless.
POLLER_PID=""
if [ -z "${RAPHA_NO_OPEN:-${ODYSSEUS_NO_OPEN:-}}" ] && command -v open >/dev/null 2>&1; then
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ Rapha is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Rapha — it will open in your browser at $URL"
