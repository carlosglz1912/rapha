...
# ── Icon (best effort) — center-crop docs/rapha.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/rapha.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/rapha.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/rapha.jpg" "$TMPIMG/sq.png"
  ...
  echo "  icon:        (skipped — no docs/rapha.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.rapha.launcher</string>
