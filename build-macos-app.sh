#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Rapha.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Rapha.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Rapha.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with RAPHA_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Rapha"
INSTALL_DIR="$REPO_DIR"
PORT="${RAPHA_PORT:-${ODYSSEUS_PORT:-7860}}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — convert the public Rapha PWA icon to .icns ──
if [ -f "$REPO_DIR/static/icons/icon-512.png" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  sips -z 512 512 "$REPO_DIR/static/icons/icon-512.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/rapha.icns" >/dev/null 2>&1; then
    echo "  icon:        rapha.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no Rapha PWA icon)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>mx.opendoctor.rapha.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>rapha</string>
...
#!/bin/bash
# Rapha.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/rapha-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Rapha\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Rapha\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Rapha isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Rapha failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Rapha is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Rapha to Applications)"
