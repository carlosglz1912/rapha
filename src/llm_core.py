...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Rapha' canonical OpenAI-style messages to native Ollama /api/chat.

    Rapha carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/carlosglz1912/rapha")
        h.setdefault("X-OpenRouter-Title", "Rapha")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Rapha-only metadata before sending messages to providers.
