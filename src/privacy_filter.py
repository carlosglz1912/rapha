...
def redact_text(text: str) -> Tuple[str, int]:
    """Return redacted text and match count."""
    if not text or not isinstance(text, str):
        return text, 0
    total = 0
    out = text
    for tag, pattern in _PATTERNS:
        total += len(pattern.findall(out))
...
def maybe_redact_messages_for_endpoint(url: str, messages: List[Dict]) -> List[Dict]:
    ...
    if not privacy_filter_enabled() or is_local_llm_url(url):
        return messages
    redacted, count = redact_messages(messages)
    if count:
        import logging
        logging.getLogger(__name__).info(
            "Privacy filter redacted %d identifier span(s) before cloud LLM call", count
        )
    return redacted