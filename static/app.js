// ============================================
// Rapha UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__raphaDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Rapha Chat';
  ...
  if (_opener) window._raphaRouteOpener = _opener;
  ...
  if (libraryNewDocBtn) {
    libraryNewDocBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      try {
        const template = await pickDocumentTemplate(libraryNewDocBtn);
        if (template && documentModule?.newDocumentFromTemplate) {
          await documentModule.newDocumentFromTemplate(template);
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('rapha-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'rapha-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Rapha...');
      }
  ...
  const UI_VIS_KEY = 'rapha-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'rapha-toolbar-visibility';
    try {
...
// ============================================
function startRaphaApp() {
  if (window.__raphaAppStarted) return;
  window.__raphaAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('rapha-doc-open-' + _curSession) === '1') {
  ...
  window._raphaBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._raphaRouteOpener) {
          try { window._raphaRouteOpener(); } catch (_) {}
          window._raphaRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startRaphaApp, { once: true });
} else {
  startRaphaApp();
}
