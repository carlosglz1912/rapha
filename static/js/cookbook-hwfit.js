...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      ...
      '',
      'Useful checks:',
      'docker compose exec rapha nvidia-smi -L',
      'docker compose exec rapha cat /proc/meminfo | head',
      'docker compose exec rapha python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed rapha. The user's selection must only change via
