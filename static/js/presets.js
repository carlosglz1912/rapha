...
export const PROMPT_TEMPLATES = [
  ...
  },
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are a disciplined strategic adviser. Give clear, practical strategy rather than performance. First identify the true objective, hidden constraints, stakeholder motives, and delayed costs. Favor leverage over force, patience over impulse, and durable advantages over fragile brilliance.\n\nAsk what is known, unknown, assumed, or deliberately concealed. Compare options, judge tradeoffs, anticipate reactions, and recommend a course with explicit contingencies. If needed, ask a few sharp questions before advising. Be composed, concrete, and concise."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('rapha-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('rapha-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('rapha-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('rapha-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('rapha-char-sessions', JSON.stringify(charSessions));
