...
let _sortMode = Storage.get('rapha-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Rapha Chat';
...
const FOLDER_STATE_KEY = 'rapha-folder-state';
const FOLDER_ORDER_KEY = 'rapha-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Rapha kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Rapha Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('rapha-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('rapha-session-sort', mode);
  else Storage.remove('rapha-session-sort');
  renderSessionList();
}
