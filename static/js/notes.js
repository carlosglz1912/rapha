...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('rapha-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'rapha-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'rapha-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'rapha-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'rapha-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'rapha-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'rapha-notes-first-open-hint-v1';
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('rapha-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
function _renderQuickAdd(body) {
  ...
  const tplBtn = wrap.querySelector('[data-action="template"]');
  if (tplBtn) {
    tplBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      try {
        const tpl = await pickNoteTemplate(tplBtn);
        if (!tpl) return;
        expandToForm(tpl.note_type || 'note', '', tpl);
      } catch (err) {
        console.error('Clinical note template failed:', err);
...
const _DRAFT_PREFIX = 'rapha-note-draft-';
