...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Rapha's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-Rapha-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-Rapha-Owner") or "").strip()
            ...
            if auth_header.startswith("Bearer "):
                ...
                if not (raw_token.startswith("rph_") or raw_token.startswith("ody_")):
                    raw_token = ""
            else:
                raw_token = ""
            if raw_token:
                # Sanity check: tokens are "rph_" or legacy "ody_" + urlsafe base64
...
async def _startup_event():
    ...
    # `RAPHA_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("RAPHA_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (RAPHA_INPROCESS_TASKS=0); "
